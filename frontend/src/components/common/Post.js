...
const Post = ({ post }) => {
	...
	const {mutate:deleteMyPost , isPending : isDeleteing} = useMutation(
		{
			mutationFn : async()=>
			{
				try {
											...
											<div key={comment._id} className='flex gap-2 items-start'>
												<div className='avatar'>
													<div className='w-8 rounded-full'>
														<img
															src={comment?.commentedOwner?.profileImg || "/avatar-placeholder.png"} alt=""
														/>
													</div>
												</div>
												<div className='flex flex-col'>
													<div className='flex items-center gap-1'>
														<span className='font-bold'>{comment?.commentedOwner?.fullName}</span>
														<span className='text-gray-700 text-sm'>
															@{comment?.commentedOwner?.userName}
														</span>
														<span className='text-gray-700 text-sm'>
															· {formattedDate}
														</span>
